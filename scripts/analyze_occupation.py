...
non_occupational_nodes = [('Q35120', 'entity'),
                          ('Q488383', 'object'),
...
def get_wd_occupation(occ, ids_dict):

    # generate graph and filter nodes
    DG = get_graph(ids_dict)
    DG.remove_nodes_from(non_occupational_nodes)

    # generate categories
    # This is terrible naming, the category thing is finding the top nodes and
    # grouping the data by them.
    ...
    top_nodes = list(map(lambda qid: get_category(qid, id_categories_dict),
                         occ.qid))
    occ['top_nodes'] = top_nodes

    # XXX: Ignoring non binary gender
    occ['total'] = occ[['male', 'female']].sum(axis=1)
    occ = occ.groupby('top_nodes').aggregate(sum)
    ...
    # Rename the occupations to the top nodes
    occ['occupation'] = occ.index
    return occ


def get_bls_wd(wd, bls_wd_match):
    ...
    bls_wd['wd_total'] = bls_wd['wd_occupation'].apply(lambda x: wd.loc[wd['occupation'].isin(x)].sum().total)
    bls_wd['wd_women'] = bls_wd['wd_occupation'].apply(lambda x: wd.loc[wd['occupation'].isin(x)].sum().female)
...
occ_data = dict()
ids_dict_dict = dict()
occ_data[2015] = pd.read_csv('/home/hargup/WIGI/analysis/occupation-index.csv')
occ_data[2016] = pd.read_csv('/home/hargup/WIGI/analysis/flat_2016_occupation-index.csv')
# occ_data[2016] = pd.read_csv('/home/hargup/WIGI/analysis/test.csv')

ids_dict_dict[2015] = json.load(open("./occ_ids.json"))
ids_dict_dict[2016] = json.load(open("./2016_occ_ids.json"))

bls_wd_match = pd.read_csv('./bls_wd_match.csv')

bls_wd_data = dict()

wd = dict()

for year in occ_data.keys():
    wd[year] = get_wd_occupation(occ_data[year], ids_dict_dict[year])
    bls_wd_data[year] = get_bls_wd(wd[year], bls_wd_match)

    df = bls_wd_data[year]
    print(year)
    print(scipy.stats.spearmanr(df['bls_p_women'], df['wd_p_women']))
    print(scipy.stats.spearmanr(df['bls_total'], df['wd_total']))
