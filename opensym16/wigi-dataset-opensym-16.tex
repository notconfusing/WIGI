...
Still there are some features lacking among the current gender indicators. The most frequently they are released is annually, which does not allow for real-time analysis. Also they measure only recent history, which limits analysis of the past. And lastly they are not open source, which means verification and remixing is more difficult.
...
Moving beyond Wikipedia data, we present 3 validation measures utilizing ground truths from the US Census Bureau, Bureau for Labor Statistics, and United Nations Development Program. Finally we demonstrate how WHGI can be utilized by revisiting and strengthening published results about gender disparities in Wikipedia.
...
\begin{table}
\caption{The relative risk for biography article quality to be misaligned with readership demand (in English Wikipedia). Here we see that biographies about women are 30\% more likely to have readership outstrip article quality (Needs Improvement), conversely biographies about men are 14\% less likely to need improvement. Spent Effort are the cases where article quality exceeds readership demand. Sitelink Ratio is the ratio of number of Wikipedia languages a biography is in comparied to the 2.6-language average for all Wikidata items.}
\begin{tabular}{lrrr}
\toprule
{} &  Needs Improv. &  Spent Effort &  Sitelink Ratio \\
\midrule
Female        &               1.30 &          1.03 &            1.29 \\
Male          &               0.86 &          1.15 &            1.29 \\
Non-binary    &               2.65 &          0.50 &            1.46 \\
Non-biography &               1.03 &          0.95 &            0.95 \\
\bottomrule
\end{tabular}
\label{table:pah}
\end{table}
...
On the other hand, using WHGI, we can calculate the average number of Wikipedia languages a biography is in by gender. Being in many languages presents a potential opportunity for quality expansion through translation. The average Wikidata item is in 2.6 Wikipedia languages, but both men and women's biographies are in 29\% more languages on average, at about 3.4 languages each. Despite the equal opportunity for translation-driven quality improvement, nevertheless Women's biographies are more likely to need improvement.
...
Yet another limitation is that there may also be biography articles in Wikipedias that are not recorded as humans in Wikidata. However the size of this set is not readily computable. Particularly we would like to know how much growth in Wikidata stems from Wikipedia's growth (e.g. a new biography is added), or migration of information to Wikidata (e.g. an existing biography in Wikipedia is marked as a human in Wikidata).
...
We wonder if tools like this would be motivating as well, as well as simply measuring progress. Zhu et al. found that Wikipedia's erstwhile ``collaboration of the week'' intiatives were effective, but failed due to the manual effort in maintaining goals \cite{zhu_organizing_2012}. This would suggest a path of research investigating how editors use information to act. As it happens Women in Red already cite WHGI for data, but would a tighter integration help the cause? A user study in providing editing communities with detailed feedback and exact movement in the landscape would ask if ``what gets measured, get fixed'' - more quickly?
