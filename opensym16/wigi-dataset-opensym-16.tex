...
We introduce a real-time, all-recorded-history level, open source dataset that can provide insights into gender disparities across time, space, culture, occupation and language. ``Wikidata Human Gender Indicators'' (WHGI) is a dataset consiting  of 11 separate indicators on the gender representation of humans in Wikidata. WHGI is open data and available for download at \url{WEBSITE BLINDED}, where we also display real-time visualizations. Our goals are to assist gender-focused Wikipedia editing communities by charting their progress, and help their cause under the philosophy of ``what gets measured, gets fixed.''  Moreover the indicators can be used in general research to measure gender disparity in ways traditional indicators can't.
...
There may also be biography articles in Wikipedias that are not recorded as humans in Wikidata. However it is not readily computable how much growth in Wikidata stems from Wikipedia's growth (e.g. a new biography is added), or migration of information to Wikidata (e.g. an existing biography in Wikipedia is marked as a human in Wikidata).
...
WHGI can have impact research by offering a large scale gender-persepctive in research. For instance Warncke-Wang et al. explored supply and demand misalignment in Wikipedia by looking at the difference in actual and predicted article quality by page-views. They cite examples of over-supplied articles where quality disproportionately exceeds readership (e.g. ``Themes in Robert Browning’s poetry''), and over-demanded articles where readership disproportionately exceeds quality (e.g. ``Wedding'', and ``cisgender''). Following this dissonance in the gender realm, they follow up by investigating misalignment in WikiProjects as a set of articles, and find that WikiProject:LGBT (Lesbian, Gay, Bisexual, Trans*) are 9.1 times more likely to be under-represented, and in need of improvement. They dub this the \textit{relative risk}.
Simply using inclusion in a WikiProject though is a very narrow scope to measure, high-level disparities. We re-ran this analysis, but instead of aggregating by WikiProject, we utilized the WHGI and were able to aggregate by gender over the entire encyclopedia. Table \ref{table:pah} shows the relative risk for gendered biographies being either in need of improvement (over-demanded), or showing extra ``spent effort'' (over-supplied). Our findings show that women's biographies are 30\% more likely to be in need of improvement compared to the average article, where as men's biographies are 14\% less likely. Women's biographies are roughly comparable to the average encyclopedia article in terms of representing spent effort in over-supplying article quality, whereas men's biographies show a 15\% higher likelihood.
The row ``non-binary'' speaks to the approximately 50 biographies whose gender is not ``male'' or ``female'', and shows about double bias. As a control, we also calculated the relative risk of every article which is not a biography, ini the ``non-biography'', which sensibly shows a close-to-baseline result, around 1 in both categories. 
Overall, the gender disparities that were allude to by  \cite{warncke-wang_misalignment_2015}, are shown to be pervasive at a high level.

This quick ``data mashup'' of the WHGI and an external data-set, is a an example of how WHGI is useful for research, in demonstring large-scale trends concerning gender.

   
\section{Discussion}
This project is not the first establish that Wikipedia data has real-world biases via correlation to exogenous indicators. In 2008 Rask concluded that Wikipedia editions displayed real-world \textit{socio-economic} disparities similar to those found in the United Nations Human Development Index (HDI) \cite{rask_reach_2008}. Rask identified three limitations with their study (1) they used only 11 Wikipedia languages (2) they suggested longitudinal analysis be carried out, and (3) languages-disaggregation is only a proxy for country-disaggregation which the HDI utilizes. WHGI overcomes all three of these limitations as it measures all 285 Wikipedia languages, updates weekly, and manages by-country-disaggregation. Even though our focus is not on socio-economic disparities, but gender disparities, the larger point of connecting Wikipedia's biases rigorously to real-world ones is shown.

We compared the number of articles added to English Wikipedia by the Women in Red project to the number of biographies added to English Wikipedia marked as women in Wikidata. The correlation between these the size of these activities was	 0.657. This medium-high correlation shows that indeed the editing levels of this group are correlated to the growth of female biographies in Wikidata. This provides a basis to use changes in WHGI as an effectiveness metric for editing. We hope to find more activity data on the content-focused editing communities, particularly non-English ones to be able to perform ``propensity score matching'' between languages. That is, for a language which has much women-focused editing we could compare the way that language's gender compostion changes against a language which was on a similar trajectory before the change, but had less women-focused editing subsequently.

Also for the indirect effects of policy changes and other initiatives that are not specifically article-creation focused we could still measure their change on biography gender. As an example, the gendered implications of changes to a notability policy could be tested using WHGI and the same propensity score matching mentioned above. We hope now that this high-level metric is being tracked it will provide an incentive to track more specific editing activity.
...
WHGI is, in essence, a biographic database. The data can not only provide insights on gender-related disparity, but also other disparities such as cultural disparities, citizen disparities and ethnic group disparities, etc.  
