...
WHGI can have impact research by offering a large scale gender-persepctive in research. For instance Warncke-Wang et al. explored supply and demand misalignment in Wikipedia by looking at the difference in actual and predicted article quality by page-views. They cite examples of over-supplied articles where quality disproportionately exceeds readership (e.g. ``Themes in Robert Browning’s poetry''), and over-demanded articles where readership disproportionately exceeds quality (e.g. ``Wedding'', and ``cisgender''). Following this dissonance in the gender realm, they follow up by investigating misalignment in WikiProjects as a whole, and that WikiProject:LGBT (Lesbian, Gay, Bisexual, Trans*) are 9.1 times more likely to be under-represented. Simply using inclusion in a WikiProject though is a very narrow scope of inclusion, and the articles in a WikiProject would likely tend to be of higher quality, since at least somebody interested in the topic has identified them. We re-run this analysis, but instead of aggregating by WikiProject, we aggregate by gender.
